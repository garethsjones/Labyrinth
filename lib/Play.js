...
var turn = require('./play/Turn'),
    ...
    instructions = require('./play/Instructions'),
    scores = require('./play/Scores');
...
module.exports = function(game, requestingPlayerId, input) {
    ...
    try {
        ...
        if (requestingPlayerId == null) {
            return EngineResponse.new(false, null, "You are a guest, available actions are 'player X', 'treasure Y' & 'shiftable'");
        }
        ...
        switch (true) {
            ...
            default:
                return EngineResponse.new(false, null, "I don't know how to '" + input + "' enter '?' for instructions");
        }
    } catch(ex) {
